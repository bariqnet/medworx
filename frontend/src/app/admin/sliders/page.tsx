  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { setEditing(null); setForm({ titleEn: '', titleAr: '', subtitleEn: '', subtitleAr: '', image: '', link: '', order: 0, isActive: true }); setShowModal(true); }} className="flex items-center gap-2 px-5 py-2.5 rounded-full btn-gradient text-white text-sm font-semibold glow-magenta">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[#141043] border border-white/[0.08] rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-8">
            <h2 className="text-xl font-bold mb-6">{editing ? 'Edit Slider' : 'Add Slider'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.subtitleEn} onChange={e => setForm({...form, subtitleEn: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white focus:border-[#c4197d] outline-none" />
                </div>
                <div>
                  <label className="block text-sm text-[#b8b5d0] mb-2">Subtitle (Arabic)</label>
                  <input value={form.subtitleAr} onChange={e => setForm({...form, subtitleAr: e.target.value})} dir="rtl" className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white focus:border-[#c4197d] outline-none font-ar" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-[#b8b5d0] mb-2">Image URL</label>
                <input value={form.image} onChange={e => setForm({...form, image: e.target.value})} required placeholder="https://..." className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white focus:border-[#c4197d] outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-[#b8b5d0] mb-2">Link (optional)</label>
                  <input value={form.link} onChange={e => setForm({...form, link: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white focus:border-[#c4197d] outline-none" />
                </div>
                <div>
                  <label className="block text-sm text-[#b8b5d0] mb-2">Order</label>
                  <input type="number" value={form.order} onChange={e => setForm({...form, order: parseInt(e.target.value)})} className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white focus:border-[#c4197d] outline-none" />
                </div>
              </div>
